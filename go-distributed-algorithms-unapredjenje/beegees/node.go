...
type BeeGeesNode struct {
	...
	aborted           map[string]bool            //UNAPREDJENJE, mapa koja pamti hash-eve blokova koji su eksplicitno odbačeni kroz AHL commit proveru (kada se pronađe equivocation proof). Ovo je uvedeno da bi se sprečilo da se na takav "otrovan" blok kasnije nadoveže novi predlog (ako je roditelj otrovan onda je i dete)

	//rotationBoundary  int             // NOVO UNAPREDJENJE: poslednji view odlučen po STAROM (punom) rasporedu cvorova
	//rotationSkipCount int             // NOVO UNAPREDJENJE: koliko je view-ova do tog trenutka "otišlo" blacklistovanom čvoru
...
func NewBeeGeesNode(id process.ProcessID, allIDs []process.ProcessID, behavior string) *BeeGeesNode {
	n := &BeeGeesNode{
		...
		aborted:     make(map[string]bool),            //UNAPREDJENJE
	}
...
func (n *BeeGeesNode) Run(ctx context.Context, inbox <-chan process.Message, send func(process.Message)) {
	...
	for {
		...
		case <-n.matTimer.C: //istek MAT timeout-a,
			...
			//fmt.Printf(" [%s] MAT Tajmer istekao! Lider kreira blok bez materijalizacije.\n", n.id)
			//n.propose(send, nil, n.newViews[n.currentView])
...
func (n *BeeGeesNode) checkCommitRule(b *Block) {
	...
	*/
	...
	// --- NOVO: Provera da li je roditelj bloka koji upravo proveravamo već abortiran ---
	if n.aborted[parent.Hash()] {
		n.aborted[b.Hash()] = true // Ako je tata otrovan, i sin je
		fmt.Printf("[%s] LANČANI ABORT: Blok %s je nevalidan jer mu je roditelj abortiran.\n", n.id, b.Hash())
		return
	}
	...
	if parent.View == grandParent.View+1 {
		...
		if n.findEquivocationProof(grandParent, parent, parent.NVset) {
			...
			n.aborted[grandParent.Hash()] = true //UNAPREDJENJE
			n.aborted[parent.Hash()] = true      // Odmah markiraj i oca kao lošeg UNAPREDJENJE
...
func (n *BeeGeesNode) getNewHeight(qc *QC) int {
	...
	if b, ok := n.blocks[qc.BlockHash]; ok {
		...
		// Ako je blok abortiran, ne koristimo ga kao osnovu
		if n.aborted[b.Hash()] {
			return n.committedHeight + 1
		}
		return b.Height + 1
...
func (n *BeeGeesNode) nextActiveAfter(id process.ProcessID, active []process.ProcessID) process.ProcessID {
	...
	for i, x := range n.allIDs {
		if x == id {
			startIdx = i //nalazimo poziciju u allIDs na kojoj se nalazi neki cvor
			break
		}
	}
	for i := 1; i <= total; i++ {
		cand := n.allIDs[(startIdx+i)%total] //kandidat je prvi sledeci aktivan cvor od tih 4 krenuvsi od cvora sa id
